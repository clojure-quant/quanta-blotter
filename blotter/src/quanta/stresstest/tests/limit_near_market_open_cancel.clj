...
(defn limit-near-market-open-cancel
  ...
  [{:keys [oms campaign quote-timeout-ms] :as this} {:keys [account/id asset qty side offset-prct]
                                                    :as order}]
  (m/sp
   (let [limit-order (m/? (near-market/near-market-limit-order
                           oms (assoc order :quote-timeout-ms quote-timeout-ms)))
