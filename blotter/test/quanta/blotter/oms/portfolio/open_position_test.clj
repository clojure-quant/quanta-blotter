...
(deftest hedge-close-overshoot-flips-keeping-position-id
  (let [{:keys [open-position]}
    ...
    (is (= closed position-closed))
    (is (not (op/position-open? closed)))
    (is (= 10M (:position/qty-entry closed)))
    (is (= 10M (:position/qty-exit closed)))
    (is (.equals 10M (:position/average-entry-price closed)))
    (is (.equals 12M (:position/avg-exit-price closed)))
    (is (= 20M (:position/realized-pl closed)))
    (is (op/position-open? opened))
    (is (= :short (:position/side opened)))
    ;; Prior long exits become short entries; prior entries become exits.
    (is (= 15M (:position/qty-entry opened)))
    (is (= 10M (:position/qty-exit opened)))
    (is (= 5M (:position/qty-open opened)))
    (is (.equals 12M (:position/average-entry-price opened)))
    (is (.equals 10M (:position/avg-exit-price opened)))
    (is (= 0M (:position/realized-pl opened)))
